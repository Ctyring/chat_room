...
void session_notify(ChatMessage::ptr msg,
                    sylar::http::WSSession::ptr session = nullptr) {
    sylar::RWMutex::ReadLock lock(m_mutex);
...
ChatWSServlet::ChatWSServlet() : sylar::http::WSServlet("chat_servlet") {}
...
int32_t ChatWSServlet::handle(sylar::http::HttpRequest::ptr header,
    ...
    SYLAR_LOG_INFO(g_logger)
        << "handle " << session << " opcode=" << msgx->getOpcode()
        << " data=" << msgx->getData();
